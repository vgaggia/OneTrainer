...
class BaseTrainingTabView(ABC):
    ...
    def build(self, column_0, column_1, column_2, controller, ui_state):
        ...
        if model_type.is_stable_diffusion_3():
            ...
            self.__setup_longcat_image_ui(column_0, column_1, column_2, controller, ui_state)
    ...
    def __setup_longcat_image_ui(self, column_0, column_1, column_2, controller, ui_state):
        self.__create_base_frame(column_0, 0, controller, ui_state)
        ...
        self.__create_noise_frame(
            column_1, 2, ui_state,
            supports_dynamic_timestep_shifting=True,
        )
    ...
    def __create_offloading_widgets(self, frame, row, ui_state, part, supports_checkpointing=True,
        ...
        if supports_activation_offloading:
            ...
            # global, not per-part, but only meaningful where activation offloading exists — and the
            # transformer is the only part that passes supports_activation_offloading, so it renders once
            self.components.label(frame, row, 0, "Compress Offloaded Activations",
                                  tooltip="Compresses offloaded activations to fp8 for the transfer to RAM, halving activation transfer traffic and pinned memory. Introduces a small quantization error into the recomputed forward pass. Only takes effect when activation offloading is enabled")
