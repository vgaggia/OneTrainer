...
from modules.model.Krea2Model import (
    ...
    KREA2_PROMPT_TEMPLATE_CROP_START,
    KREA2_SELECT_LAYERS,
    PROMPT_MAX_LENGTH,
    Krea2Model,
)
...
class Krea2BaseDataLoader(
    ...
    def _preparation_modules(self, config: TrainConfig, model: Krea2Model):
        ...
        tokenize_prompt = Tokenize(in_name='prompt', tokens_out_name='tokens', mask_out_name='tokens_mask', tokenizer=model.tokenizer, max_token_length=PROMPT_MAX_LENGTH,
                                   format_text=KREA2_PROMPT_TEMPLATE, additional_format_text_tokens=KREA2_PROMPT_TEMPLATE_CROP_START)
        # Krea 2 stacks 12 Qwen3-VL hidden-state layers; EncodeQwenText concatenates
        # the requested layers along the feature axis -> (L, 12*2560).
        encode_prompt = EncodeQwenText(tokens_name='tokens', tokens_attention_mask_in_name='tokens_mask', hidden_state_out_name='text_encoder_hidden_state', tokens_attention_mask_out_name='tokens_mask',
                                       text_encoder=model.text_encoder, hidden_state_output_index=list(KREA2_SELECT_LAYERS), autocast_contexts=[model.autocast_context], dtype=model.train_dtype.torch_dtype(), crop_start=KREA2_PROMPT_TEMPLATE_CROP_START)
    ...
    def _create_dataset(
        ...
        return DataLoaderText2ImageMixin._create_dataset(self,
            ...
            vae_frame_dim=True,  #...the Qwen-Image VAE is video-capable. convert images to video dimensions
        )

factory.register(BaseDataLoader, Krea2BaseDataLoader, ModelType.KREA_2)
